...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-dbecb151";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-dbecb151">'+'<span class="tile-face-dbecb151 tile-front-dbecb151"></span>'+'<span class="tile-face-dbecb151 tile-back-dbecb151"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dbecb151")){return;}
var back=tile.querySelector(".tile-back-dbecb151");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-dbecb151");if(isMine){tile.classList.add("boom-dbecb151");}
if(dim){tile.classList.add("dimmed-dbecb151");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-dbecb151")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-dbecb151");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dbecb151")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-dbecb151");void panelEl.offsetWidth;panelEl.classList.add("jolt-dbecb151");}
endRound();}
